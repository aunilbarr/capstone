package com.example.fayyadhaunilbarr.core.utils
...
object DataMapper {
    ...
    fun mapEntitiesToDomain(input: List<com.example.fayyadhaunilbarr.core.data.source.local.entity.movie>): List<MovieModel> {
        val movieList = ArrayList<MovieModel>()
        input.map {
            val movie = MovieModel(
                it.id,
    ...
    fun mapDomainToEntity(input: MovieModel): com.example.fayyadhaunilbarr.core.data.source.local.entity.movie {
        return com.example.fayyadhaunilbarr.core.data.source.local.entity.movie(
            input.id,
            input.title,
            input.releaseDate,
            input.overview,
