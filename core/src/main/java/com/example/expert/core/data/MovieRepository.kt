...
class MovieRepository(
    ...
    override fun getFavoriteMovies() =
        localDataSource.getFavorite().map { DataMapper.mapEntitiesToDomain(it) }

    override fun checkFavoriteMovie(id: Int): LiveData<Boolean> {
        val liveData = MutableLiveData<Boolean>()
        localDataSource.check(
            id,
            object :
                com.example.expert.core.data.source.local.LocalDataSource.CheckFavoriteCallback {
    ...
    override fun insertMovie(movie: MovieModel) =
        localDataSource.insert(DataMapper.mapDomainToEntity(movie))

    override fun deleteMovie(movie: MovieModel) =
